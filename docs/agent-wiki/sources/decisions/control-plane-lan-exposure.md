...
4. 手机通过**设备配对**进入，不存在「扫码即登录」。Mac 上有一个白名单页面：发起配对
   → 手机扫码 → **手机上显示 6 位配对码** → 在 Mac 上输入该码才完成绑定。绑定产物是一条
   具名、可撤销的白名单记录，磁盘上只存 token 的 SHA-256。

第 4 条是后加的，它取代了最初「扫码直接换一个只读会话」的做法。那个做法有三个问题：
二维码本身就是 bearer secret，被拍照或投屏即等于泄漏，而扫码即授权意味着泄漏无声无息；
会话是匿名的，无法回答「现在有几台手机连着」；会话不可撤销，只能等它过期。要求配对码在
**Mac 上**输入，等于要求操作者同时掌握两块屏幕，而「判定」也就发生在白名单所在的可信面。
